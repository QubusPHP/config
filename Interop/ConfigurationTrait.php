...
namespace Qubus\Config\Interop;

use ArrayAccess;
use Qubus\Config\Interop\Exception;
use Traversable;
...
trait ConfigurationTrait
{
     ...
     */
    public function canRetrieveOptions(array|ArrayAccess $config, ?string $configId = null): bool
    {
        $dimensions = $this->iterableToArray($this->dimensions());
     ...
     */
    public function options(array|ArrayAccess $config, ?string $configId = null): array|ArrayAccess
    {
        $dimensions = $this->iterableToArray($this->dimensions());
        ...
        if ($this instanceof ProvidesDefaultOptions) {
            ...
            $config = array_replace_recursive($this->iterableToArray($options), (array)$config);
        }
        return $config;
    }
     ...
     * @throws Exception\MandatoryOptionNotFoundException If a mandatory option is missing
     */
    public function optionsWithFallback(array|ArrayAccess $config, ?string $configId = null): array|ArrayAccess
    {
        $options = [];
        ...
        if ($this->canRetrieveOptions($config, $configId)) {
            ...
            $options = $this->iterableToArray($this->defaultOptions());
        }
        return $options;
    }
     ...
     */
    private function checkMandatoryOptions(iterable $mandatoryOptions, array|ArrayAccess $config): void
    {
        foreach ($mandatoryOptions as $key => $mandatoryOption) {
            ...
            if ($useRecursion && isset($config[$key])
                && (is_array($config[$key]) || $config[$key] instanceof ArrayAccess)
            ) {
    ...
    private function iterableToArray(iterable $values): iterable
    {
